...
from re import compile, sub, match as rematch
...
telegram_uid_regex = r"\d+" # (?<!\d)\d{6,9}(?!\d)
...
async def get_entity(bot, ctx) -> Any | str:
    if ctx.input:
        ...
        try:
            entity = await bot.client.get_entity(entity_ref)
...
def linkify_fullname(fullname):
    return sub(r'[^a-zA-Z0-9]', '_', fullname)
...
def split_args(input_str: str) -> list[str]:
    """Split a string into arguments, respecting quoted sections.
    
    Args:
        input_str: String to split into arguments
        
    Returns:
        List of argument strings
        
    Example:
        >>> split_args('arg1 "arg 2" arg3')
        ['arg1', 'arg 2', 'arg3']
    """
    args = []
    current_arg = []
...
def parse_timespan(timespan: str) -> timedelta:
    ...
    Supports units: s (seconds), m (minutes), h (hours), d (days), w (weeks), M (months), y (years)
    Example inputs: "1s", "15m", "6h", "12d", "1w", "2M", "1y"
    """
    if not timespan:
        raise ValueError(f"No timespan given!")
        
    # Regular expression to match number + unit
    match = rematch(r"^(\d+)([smhdwMy])$", timespan)
    if not match:
    ...
    if unit == "s":
        ...
        return timedelta(days=amount * 30)  # Approximate months as 30 days
    elif unit == "y":
        return timedelta(days=amount * 365)  # Approximate years as 365 days
    else:
