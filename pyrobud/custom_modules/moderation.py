...
class ModerationModuleAddon(module.Module):
    ...
    async def cmd_globalban(self, msg: tg.events.newmessage, *users: tuple):
        ...
        if ctx.msg.is_reply:
            replied_msg = await ctx.msg.get_reply_message()
    ...
    async def cmd_purgemessages(self, msg: tg.events.newmessage):  # , amount: int = None
        await self.bot.client.delete_messages(ctx.msg.chat_id, [x for x in range(ctx.msg.id)], revoke=True)
        await ctx.msg.result(f"Purged last {msg.id} messages!")
