...
class DeleteLog(module.Module):
    ...
    async def on_message_deleted(self, msg: tg.events.MessageDeleted):
        ...
        chat = util.bluscream.ChatStr(
            await self.bot.client.get_input_entity(msg.original_update.channel_id)) if haschat else "an Unknown chat"
        msg_ids = '`, `'.join(str(x) for x in msg.deleted_ids)
        await self.bot.client.send_message("Deleted Messages",
                                           f"Someone deleted {len(msg.deleted_ids)} message(s) from {chat}\nMessages: `{msg_ids}`")
        """
