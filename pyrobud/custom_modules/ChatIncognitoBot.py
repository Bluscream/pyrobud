...
class ChatIncognitoBot(module.Module):
    ...
    async def on_message(self, msg: tg.custom.Message):
        ...
        if msg.from_id == self.bot.uid: # self -> bot
            ...
            # print("check_for_media:", self.check_for_media, "has_affecting_media:", has_affecting_media(msg), "startswith(Emojis.Other.AuthorMessage):", ctx.msg.text.startswith(Emojis.Other.AuthorMessage))
            if self.check_for_media and has_affecting_media(msg) and not ctx.msg.text.startswith(Emojis.Other.AuthorMessage): # Media
                _now = datetime.now()
            ...
            if r_match: # Partner Message
                ...
                if Emojis.Session.end in msg.text:
                    print("Emojis.Session.end in ctx.msg.text")
                ...
                """
                for char in list(ctx.msg.text): # todo: remove debug shit
                    print(char, "==", Emojis.Session.start, ":", char == Emojis.Session.start, char.encode('ascii', 'backslashreplace'))
                """
                print(msg.text)
                print("Emojis.Session.start in ctx.msg.text ==", Emojis.Session.start in ctx.msg.text)

                if Emojis.Session.start in msg.text:
                    print("Emojis.Session.start in ctx.msg.text")
                    self.newSession(msg)
                    if not self.activeSession.greeted and choice(True, False): await self.greet(ctx.msg, self.activeSession)
                    print("new session!")
                if Emojis.Session.searching in msg.text:
                    print("Emojis.Session.searching in ctx.msg.text")
                    if self.sessionActive: self.activeSession.close()
