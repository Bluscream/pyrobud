...
from .. import module, command, util
...
"""
    ...
    async def on_message(self, msg: tg.custom.Message):
        ...
        if not txt.startswith(self.secommand): return
        now = datetime.now()
        ...
        text = text[len(self.bot.prefix + self.secommand):].replace("`", "").strip()
        ...
        await ctx.msg.reply(self.template.format(command=str(text).replace("`", ""), result=str(result).replace("`", ""), time=el_str))

    async def cmd_supereval(self, ctx: command.Context, args: str):
        if args:
            ...
            if args[0] == "time":
                ...
                repl: tg.custom.Message = await ctx.msg.get_reply_message()
            ...
            if ctx.msg.to_id in self.enabled_for:
                self.enabled_for.remove(ctx.msg.to_id)
            else:
                self.enabled_for.append(ctx.msg.to_id)
            state = "enabled" if ctx.msg.to_id in self.enabled_for else "disabled"
