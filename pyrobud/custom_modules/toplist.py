...
class TopListModule(module.Module):
    ...
    async def parse_message(self, msg: tg.custom.Message):
        ...
        """
        matches = self.no_mention_pattern.finditer(ctx.msg.text, MULTILINE | IGNORECASE)
        new_txt: str = ctx.msg.text
    ...
    @command.alias("top")
    async def cmd_toplist(self, ctx: command.Context, count: int = 5):
        count = int(count) if count else 5
        chat: Union[tg.types.Chat, tg.types.Channel] = await ctx.msg.get_chat()
